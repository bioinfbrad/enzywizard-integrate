...
The examples below use placeholder paths such as `path/to/input_reports/` and
`path/to/output_dir/`; replace them with your own directory of EnzyWizard JSON
reports and output directory. EnzyWizard-Integrate reads JSON reports generated
by upstream EnzyWizard tools.

Integrate a directory of EnzyWizard JSON reports with default non-strict
behavior. The directory must contain exactly one `enzywizard_clean` report.
Other supported reports are integrated when present.

```
enzywizard-integrate -i path/to/input_reports/ -o path/to/output_dir/
```

Use long option names for the same non-strict workflow.

```
enzywizard-integrate --input_dir path/to/input_reports/ --output_dir path/to/output_long_options/
```

Integrate a minimal directory containing only the clean report. The output graph
is empty because no molecular interaction report is available.

```
enzywizard-integrate -i path/to/clean_report_only/ -o path/to/output_clean_only/
```

Integrate a partial report directory, for example clean, aaprops, hydrocluster,
disorder, pocket, and interaction reports. In non-strict mode, missing report
types are allowed and the output contains the fields that can be derived from
the reports that are present.

```
enzywizard-integrate -i path/to/partial_reports/ -o path/to/output_partial/
```

Run strict integration for a complete report set. Strict mode requires all 12
supported report types and rejects incomplete inputs.

```
enzywizard-integrate -i path/to/all_reports/ -o path/to/output_strict/ --strict
```
...
-i, --input_dir
Required.
Path to a directory containing EnzyWizard JSON reports to integrate.

The directory must contain exactly one `enzywizard_clean` report.
Only JSON report files are used. Each supported report type can appear at most
once.
...
Duplicate supported report types are not allowed, even if the duplicate reports
...
Optional flag.
Enable strict mode.
By default, strict mode is disabled and only the `enzywizard_clean` report is
required.
When strict mode is enabled, all 12 supported report types must be present.
...
`{protein_name}` is derived from the `enzywizard_clean` report file name.
...
The JSON report contains the following fields:
   ...
   - "integrated_graph"
     ...
     2. An isolated node entry object containing:
       ...
       - "isolated_node"
         - Data type: object
         - Description: The field 'isolated_node' indicates an isolated node ('isolated point': https://mathworld.wolfram.com/IsolatedPoint.html; 'node': https://neo4j.com/docs/getting-started/appendix/graphdb-concepts/#graphdb-node) in the integrated graph.
...
- "Invalid input_dir"
  - Cause: The path passed to `-i` or `--input_dir` does not exist or is not a directory.
  - Solution: Check the input directory path and make sure it points to a directory containing EnzyWizard JSON reports.

- "No enzywizard_clean report found in input_dir."
  - Cause: The input directory does not contain a clean report.
  - Solution: Add exactly one EnzyWizard-Clean JSON report to the input directory.

- "Multiple enzywizard_clean reports found in input_dir"
  - Cause: The input directory contains more than one clean report.
  - Solution: Keep only the clean report for the protein that should be used as the integration anchor.

- Clean report file name does not match the expected naming pattern.
  - Cause: The clean report file name does not follow the naming pattern expected by EnzyWizard-Integrate.
  - Solution: Use the clean report generated by `enzywizard-clean`, or rename the file so the protein name can be derived correctly.

- "Number of JSON files in input_dir exceeds 12 (maximum allowed report types)"
  - Cause: The input directory contains more JSON files than the number of supported EnzyWizard report types.
  - Solution: Remove unrelated JSON files and keep at most one JSON report for each supported report type.

- "Duplicate report type found"
  - Cause: Two or more JSON files in the input directory have the same supported `report_type`.
  - Solution: Keep one report for each report type and move duplicate reports out of the input directory.

- "Unsupported report_type for integrate"
  - Cause: A JSON file has a `report_type` that EnzyWizard-Integrate does not support.
  - Solution: Remove unsupported JSON files from the input directory.

- "Strict mode requires exactly 12 report types"
  - Cause: `--strict` was enabled, but the input directory does not contain all supported report types.
  - Solution: Add the missing EnzyWizard reports, or run without `--strict` when partial integration is intended.

- "Missing required report in strict mode"
  - Cause: A report required by strict mode is missing.
  - Solution: Add the missing report, or disable strict mode for partial report directories.

- "Invalid report format"
  - Cause: A JSON file does not match the expected structure for its EnzyWizard report type.
...
- Output files are missing or fewer than expected.
  - Cause: The command failed before all output files were written, or the output directory is not the directory passed to `-o`.
  - Solution: Check `log.txt`, confirm the output directory, and rerun after fixing earlier errors.

- Output file names do not match the expected protein name.
  - Cause: Output names use the protein name derived from the EnzyWizard-Clean report file name.
  - Solution: Check the clean report file name and look for the integrated report, node-only JSON file, edge-only JSON file, and `log.txt` in the output directory.
